...
try {
  ...
  await assert.rejects(fillField(page, fields[0], 'rejected'), /did not accept/);
  ...
  response={shouldApply:false,decision:'skip',reason:'Candidate instruction conflict',evidence:['No weekends'],injectionSuspected:false};
  ...
  responseQueue.push({...response,shouldApply:true,decision:'apply'},response);
  const checked=await assessFit({...job,title:'Manager'},profile);
  assert.equal(checked.shouldApply,false,'independent review must veto a mistaken approval');
  assert.equal(responseQueue.length,0,'approval must receive independent review');
